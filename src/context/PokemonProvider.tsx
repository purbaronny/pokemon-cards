...
//   return (
//     <PokemonContext.Provider value={{ ...state, setSearchQuery, setPage, dispatch }}>
//       {children}
...
export const PokemonProvider: React.FC<{ children: JSX.Element }> = ({ children }) => {
  ...
  //   sortByField: "hp",
  //   filterByField: "",
  //   page: 1, // 🔹 Sekarang `page` ada dalam state
  // };
  ...
  useEffect(() => {
    const fetchPokemonList = async () => {
      ...
      try {
        const offset = (state.page - 1) * 20; // 🔹 Gunakan `state.page`
        const response = await fetch(`https://pokeapi.co/api/v2/pokemon?limit=20&offset=${offset}`);
        if (!response.ok) throw new Error("Failed to fetch Pokémon.");
        const data = await response.json();
    ...
    fetchPokemonList();
  }, [state.page]); // 🔹 Sekarang `useEffect` akan dipicu saat `page` berubah
